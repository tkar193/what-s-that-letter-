function [image_letters, image_words, BBoxes, textBBoxes, cellLetters, cellWords] = segmentation(image, smallest)
    I = image;
...
%     mserRegions(filterIdx) = [];
    ...
    image_letters = insertShape(image,'Rectangle',BBoxes,'LineWidth',3);
    cellLetters = cell(size(BBoxes,1), 1);
    for x = 1:size(BBoxes,1)
        cellLetters{x} = imcrop(image, BBoxes(x,:));
    end
...
%     image_words = insertShape(image, 'Rectangle', textBBoxes, 'LineWidth', 3);
%     cellWords = cell(size(textBBoxes,1),1);
%     for x = 1:length(textBBoxes)
%         cellWords{x} = imcrop(image, textBBoxes(x,:));
%     end
    cellWords = 0;
    image_words = 0;
    textBBoxes = 0;
